...
``chunk_frames``) are pure and unit-tested. The BlueZ glue (``serve_forever``,
...
# ---- Pure framing core (shared by every link, unit-tested) ----
...
def serve_forever(registry) -> None:  # pragma: no cover (hardware)
    import dbus
    import dbus.service
...
def serve_forever_ble(registry, adapter: str = "hci0") -> None:  # pragma: no cover (hardware)
